...
select O.o_id as `order id`, O.o_date as `order date`, O.o_total_price as `price`
from `order` O;

-- Hiển thị danh sách các khách hàng đã mua hàng, và danh sách sản phẩm được mua bởi các khách
select C.c_name, P.p_name from
customer C join `order` O on C.c_id = O.c_id 
...
select O.o_id, O.o_date, OD.od_qty * P.p_price as total_price
